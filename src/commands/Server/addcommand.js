...
class addCommand extends Command {
    ...
    async run(message, args, kimiwa) { // eslint-disable-line no-unused-vars
        ...
        if (name === false) return message.channel.createEmbed(new kimiwaHelper.Embed().setAuthor("ERROR", message.author.avatarURL).setDescription(`Thanks to asigne name to your command with --name [name of command]`));
        if (value === false) return message.channel.createEmbed(new kimiwaHelper.Embed().setAuthor("ERROR", message.author.avatarURL).setDescription(`Thanks to asigne value to your command with --value [value of command]`));
        ...
        try {
            ...
            message.channel.createEmbed(new kimiwaHelper.Embed()
                ...
                .setDescription(`The command **${name}** has create for this server`)
            )
        } catch (error) {
            return message.channel.createEmbed(new kimiwaHelper.Embed()
                .setAuthor("ERROR", message.author.avatarURL)
