...
class Recent extends Command {
    ...
    async run(message, args, kimiwa, level, IA) { // eslint-disable-line no-unused-vars
        ...
        console.log(acc);
        let beatmapPP = kimiwaHelper.ojsama.ppv2({stars: beatmapStars, max_combo: Number(getRecent[0].maxcombo), n100: Number(getRecent[0].count100), n50: Number(getRecent[0].count50), nmiss: Number(getRecent[0].countmiss), acc_percent: acc });
